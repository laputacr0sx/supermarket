...
def test_upc_a_pads_to_ean13() -> None:
    assert normalize("012345678905") == "0012345678905"


def test_spaces_and_dashes_stripped() -> None:
    assert normalize(" 489-0000 00001 ") == "0489000000001"


def test_ean13_check_vector() -> None:
...
def test_mint_is_not_sequential() -> None:
...
def test_mint_respects_existing() -> None:
    first = mint_store_ean13(set())
    second = mint_store_ean13({first})
    assert first != second


def test_same_raw_forms_normalize_equal() -> None:
