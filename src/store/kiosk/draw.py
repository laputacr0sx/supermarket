...
def paint(pygame: Any, fonts: Fonts, dest: Any, vm: ViewModel) -> None:
    ...
    if vm.flash == "ok":
        ...
        bg = SOFT_BG
    dest.fill(bg)
    bar = AMBER if vm.header == COPY.staff_header else WOOD
    ...
    if vm.tag_yuan is not None:
        tag = fonts.render(f"{vm.tag_yuan}{COPY.yuan}", 96, HKRED)
    ...
    if vm.sum_yuan is not None:
        label = fonts.render(f"{vm.sum_label} {vm.sum_yuan}{COPY.yuan}", 64, HKRED)
